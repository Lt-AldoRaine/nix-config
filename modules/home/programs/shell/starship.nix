{ lib, ... }: {
  # Jetpack preset (https://starship.rs/presets/jetpack) - uses the
  # terminal's own ANSI color theme, so it stays in sync with wallust
  # automatically without needing hex color templating.
  #
  # Disable stylix's starship target: it injects a static
  # `palette`/`palettes.base16` table that would shadow ANSI names (like
  # "bright-yellow") with fixed hex values from the build-time theme,
  # breaking wallust's live per-wallpaper recoloring.
